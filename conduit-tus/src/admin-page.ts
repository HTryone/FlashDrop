...
// 全部挂在 Worker 上，密码用 SHA-256 哈希存 KV（只存哈希），每次请求带 X-Admin-Password 头。
// 控制台零外部依赖、零构建步骤；凭据默认走仓库写法（见 storage-router），KV bucket_cfg 仅作覆盖层。
...
const PAGE_CSS = `
  ...
  *{box-sizing:border-box} body{margin:0;background:var(--bg);color:var(--fg);font:14px/1.6 system-ui,'Microsoft YaHei',sans-serif}
  ...
  code{background:#0d1117;padding:1px 5px;border-radius:4px;color:var(--blue)}`;

function adminHtml(setup: boolean): string {
...
<h1>中转站配额管理 · 站长后台</h1>`;
  ...
  if (setup) {
    return (
      head +
      `<div class="card"><h2>首次设置密码</h2>
       <p class="muted">尚未设置控制页密码，请先设置（全局共用一把锁）。</p>
       <div class="row"><input id="pw" type="password" placeholder="至少6位"/>
       <button onclick="setPw()">设置密码</button></div>
       <p id="msg" class="muted"></p></div>
       <script>
       async function setPw(){var pw=document.getElementById('pw').value;
         if(pw.length<6){document.getElementById('msg').textContent='密码至少6位';return;}
         var r=await fetch('/api/admin/password',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({password:pw})});
         if(r.ok){location.reload();}else{document.getElementById('msg').textContent='设置失败';}}
       </script></body></html>`
    );
  }
...
export async function handleAdmin(request: Request, ctx: AdminCtx): Promise<Response> {
  ...
  const pwHash = ctx.kv ? await ctx.kv.get('quota:admin_password_hash') : null;

  if (path === '/admin') {
    if (request.method === 'GET') return new Response(adminHtml(pwHash ? false : true), { headers: { 'Content-Type': 'text/html; charset=utf-8' } });
  ...
  if (path.startsWith('/api/admin/')) {
    ...
    // 设密码：首次或显式修改，无需旧密码校验
    if (api === 'password' && request.method === 'POST') {
      const body = (await request.json().catch(() => ({}))) as { password?: string };
      if (!body.password || body.password.length < 6) return json({ error: '密码至少6位' }, origin, 400);
      if (ctx.kv) await ctx.kv.put('quota:admin_password_hash', await sha256Hex(body.password));
    ...
    if (api === 'config' && request.method === 'POST') {
      ...
      if (ctx.kv) {
        if (typeof body.enabled === 'boolean') await ctx.kv.put(`quota:${body.account_id}:enabled`, body.enabled ? 'true' : 'false');
        if (typeof body.limit_bytes === 'number') await ctx.kv.put(`quota:${body.account_id}:limit_bytes`, String(body.limit_bytes));
      }
      return json({ ok: true }, origin);
    }
