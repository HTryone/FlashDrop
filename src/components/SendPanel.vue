...
import { useLocalSend } from '@/composables/useLocalSend';

const emit = defineEmits<{
  (e: 'gotLoginCode', code: string): void;
}>();

// 发送方式：中转发送 | 本地直传（视图标签态，仅控制显示哪块 UI，非业务逻辑）
...
// 中转发送状态机（TUS）已抽到 src/transfer/tus/useRelayTransfer.ts
const {
...
} = useLocalSend(files, clearSelected);

// 顶部总状态条：纯展示切换（业务三态推导已在 composable 的 sendStatus 与 useRelayTransfer 的 selStatus 内）
const senderStatus = computed(() => sendMode.value === 'local' ? sendStatus.value : selStatus.value);
const senderStatusClass = computed(() => sendMode.value === 'local' ? sendStatusClass.value : selStatusClass.value);
</script>
    ...
    <div class="local-send-panel">
      ...
      <div v-else class="roominfo">
        ...
        <div class="actions">
          ...
          <span class="mode-badge">{{ localTransport === 'p2p' ? 'P2P 直连' : 'HTTP 中继' }}</span>
        </div>
