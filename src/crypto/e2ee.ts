...
// 分片越大 → 帧数越少 → 帧头/加密/WS 开销越低 → 吞吐越高。
// CF DO WebSocket 单条消息上限 16MB（≈16,777,216 字节）。
// 4MB 加密后 ≈ 4.06MB，余量充足；272MB 文件只需 ~68 帧（原 768KB 时 363 帧）。
export const LOCAL_CHUNK_SIZE = 4 * 1024 * 1024; // 4MB 一块
